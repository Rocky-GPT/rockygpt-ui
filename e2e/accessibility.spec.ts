...
test('the composer stays inside the band the keyboard leaves visible', async ({ page }) => {
  const composer = page.getByRole('textbox', { name: 'Message RockyGPT' });
  ...
  await page.evaluate((band) => {
    // Stand in for the keyboard. No engine Playwright drives has a software
    // one, so the band is published by hand — and published the way Safari
    // publishes it: not merely shorter, but pushed *down* the page, which is
    // what it does to bring a focused field near the bottom into view.
    const style = document.documentElement.style;
    style.setProperty('--viewport-height', `${band}px`);
    style.setProperty('--viewport-top', `${window.innerHeight - band}px`);
  }, BAND);

  // The composer must come to rest within the top `--viewport-height` pixels
  // of the page: the band as Safari shows it before scrolling anywhere. Below
  // that lies a region reachable only by scrolling the band off the header and
  // the whole greeting — which is what put a lone text field on a blank
  // screen. Staying up here is what gives Safari nothing to scroll to.
  await expect.poll(bottomOf).toBeLessThanOrEqual(BAND);
  await expect.poll(bottomOf).toBeGreaterThan(BAND - 200);
});
